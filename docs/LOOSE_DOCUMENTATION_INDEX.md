...
| [`Arduíno.txt`](../Ardu%C3%ADno.txt) | dossiê AVR/registradores | `TECHNICAL_REFERENCE_DRAFT` | refatorar para laboratório de hardware com datasheet + build + medição |
| [`Arm64 Mixer leve criptografia.md`](../Arm64%20Mixer%20leve%20criptografia.md) | transcrição/experimento AArch64 NEON | `EXPERIMENTAL_REVIEW` | separar código, teste e benchmark; não promover como criptografia sem prova |
...
|---|---|---|---|
| [`RAF_INDEX.md`](../RAF_INDEX.md) | índice físico dos 56 métodos `RAF_0xx_*.c` | `DOMAIN_INDEX` | distinto de `docs/INDEX.md`; pode permanecer próximo dos fontes enquanto o layout for flat |
...
| [`RAF_56_METHODS.md`](../RAF_56_METHODS.md) | catálogo conceitual dos 56 métodos | `REFERENCE / CLAIM_REVIEW` | valores de “ganho estimado” permanecem estimativas até benchmark observável |
| [`RAF_BENCHMARK_MATRIX.md`](../RAF_BENCHMARK_MATRIX.md) | matriz estimado vs real | `PENDING_MEASUREMENT` | `_medir_` deve ser interpretado como `TOKEN_VAZIO_MEASUREMENT` |
| [`RAF_CHECKLIST_96_ITEMS.md`](../RAF_CHECKLIST_96_ITEMS.md) | checklist de estratégias + métodos | `AUDIT / RECONCILE_STATES` | `[x]` pode representar existência, compile, selftest ou runtime; requer vetor de estados explícito |
...
## 5. Critério para promoção ou movimentação
...
Depois, reconciliar este índice com o snapshot gerado do commit corrente e atualizar decisões máquina-legíveis somente de forma append-only/evolutiva.
...
**F_ok:** existe governança documental, decisões explícitas e preservação de proveniência.  
**F_gap:** cobertura e snapshots gerados podem estar defasados em relação ao `main`; existência física não resolve ligação semântica.  
**F_next:** regenerar catálogo/mapa no commit corrente, reconciliar estados e só então promover ou mover arquivos.