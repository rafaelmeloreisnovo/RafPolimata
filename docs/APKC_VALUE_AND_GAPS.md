...
>
> **Governança de lacunas:** `CLOSURE_L1` (proveniência/reprodutibilidade),
> `CLOSURE_L2` (runtime Android) e `CLOSURE_L3` (validação ARM64).
...
| Runtime NativeActivity | `logcat` filtrado sem crash | TOKEN_VAZIO — `CLOSURE_L2` |
...
| **US$ 100k–300k** | Tudo acima **+** runtime ARM64 logcat sem crash (L2) **+** ARM64 ELF no APK (L4) **+** transcript source→binary (L1) | **PENDING** — L1 source→binary está **PASS** na rodada limpa de 2026-08-16; L2 runtime segue `TOKEN_VAZIO` (`CLOSURE_L2`) e L4 empacotamento ARM64 segue `TOKEN_VAZIO` (`CLOSURE_L3`) |
| **US$ 300k–750k** | Tudo acima **+** CI artifact verde (run completo com upload, step "Upload ApkC proof artifacts", `.github/workflows/ci.yml:103-109`) **+** 3+ linguagens **provadas em runtime** (não só framework) via `scripts/apkc_lang_coverage.sh` (step `.github/workflows/ci.yml:57-58`) | **PENDING** — coverage roda 6/12 mas runtime por linguagem é `TOKEN_VAZIO` (`CLOSURE_L2`); ARM64 no APK ainda não fecha a célula (`CLOSURE_L3`) |
| **US$ 1M+** | Tudo acima **+** adoção externa **+** corpus regressivo de formatos (`tests/fixtures/`, L17) **+** release navegável (L19) | **PENDING/TOKEN_VAZIO** — corpus regressivo aberto (`docs/LACUNAS...` L17), release navegável inexistente (`docs/LACUNAS...` L19); governança de evidência preservada por `CLOSURE_L1` |
...
- **API/ABI matrix** — minSdkVersion provado nos bytes do AXML
  ...
  células sem toolchain ARM ficam `TOKEN_VAZIO` honestamente — `CLOSURE_L3`
...
> Cada avanço técnico (sair de TOKEN_VAZIO via `CLOSURE_L1`/`CLOSURE_L2`/`CLOSURE_L3` num gate nomeado acima) move a tese
