...
# Hosted Ubuntu clang defaults to PIE even for -nostdlib. That produced a PT_INTERP
# for /lib/ld-linux-aarch64.so.1 and made the supposedly freestanding proof depend
# on a target sysroot. Require a static, non-PIE AArch64 executable instead.
A64_LINK=(-static -no-pie -fuse-ld=lld -Wl,-e,_start -Wl,--build-id=none)
A64_CMD=("${COMMON[@]}" --target=aarch64-linux-gnu "${A64_LINK[@]}" "$SRC" -o "$A64_1")
A64_CMD_2=("${COMMON[@]}" --target=aarch64-linux-gnu "${A64_LINK[@]}" "$SRC" -o "$A64_2")
...
if "${A64_CMD[@]}" 2>"$A64_ERR"; then
    ...
    if "${A64_CMD_2[@]}" 2>>"$A64_ERR"; then
        A64_SHA="$(sha256sum "$A64_1" | awk '{print $1}')"; A64_SHA_2="$(sha256sum "$A64_2" | awk '{print $1}')"
        if [ "$A64_SHA" = "$A64_SHA_2" ] && cmp -s "$A64_1" "$A64_2"; then A64_REPRO="PASS"; else A64_REPRO="FAIL"; fi
    else A64_REPRO="FAIL"; fi
else A64_BUILD="FAIL"; fi

{
    printf '[A64] build=%s identity=%s static_no_interp=%s reproducibility=%s sha256=%s\n' "$A64_BUILD" "$A64_IDENTITY" "$A64_STATIC" "$A64_REPRO" "$A64_SHA"
    if [ -s "$A64_ERR" ]; then printf '[A64] stderr (first 20 lines):\n'; head -20 "$A64_ERR"; fi
    printf '\n[A32] command:\n'; print_cmd "${A32_CMD[@]}"
} >> "$TRANSCRIPT"

if "${A32_CMD[@]}" 2>"$A32_ERR"; then
    A32_BUILD="PASS"; "$ELF_READER" -h "$A32_1" > "$A32_HDR"
    if grep -Eq 'Class:[[:space:]]+ELF32' "$A32_HDR" && grep -Eq 'Machine:[[:space:]]+ARM' "$A32_HDR"; then A32_IDENTITY="PASS"; else A32_IDENTITY="FAIL"; fi
    if "${A32_CMD_2[@]}" 2>>"$A32_ERR"; then
        A32_SHA="$(sha256sum "$A32_1" | awk '{print $1}')"; A32_SHA_2="$(sha256sum "$A32_2" | awk '{print $1}')"
        if [ "$A32_SHA" = "$A32_SHA_2" ] && cmp -s "$A32_1" "$A32_2"; then A32_REPRO="PASS"; else A32_REPRO="FAIL"; fi
    else A32_REPRO="FAIL"; fi
else A32_BUILD="FAIL"; fi

{
    printf '[A32] build=%s identity=%s reproducibility=%s sha256=%s\n' "$A32_BUILD" "$A32_IDENTITY" "$A32_REPRO" "$A32_SHA"
    if [ -s "$A32_ERR" ]; then printf '[A32] stderr (first 20 lines):\n'; head -20 "$A32_ERR"; fi
} >> "$TRANSCRIPT"

COMPLETE=0
if [ "$A64_BUILD" = PASS ] && [ "$A64_IDENTITY" = PASS ] && [ "$A64_STATIC" = PASS ] && [ "$A64_REPRO" = PASS ] && [ "$A32_BUILD" = PASS ] && [ "$A32_IDENTITY" = PASS ] && [ "$A32_REPRO" = PASS ]; then COMPLETE=1; fi
if [ "$COMPLETE" -eq 1 ]; then FINAL_STATE="PASS"; else FINAL_STATE="FAIL"; fi
...
cat > "$STATUS_JSON" <<JSON
{
  ...
  "aarch64": {"build":"$A64_BUILD","identity":"$A64_IDENTITY","static_no_interp":"$A64_STATIC","reproducibility":"$A64_REPRO","sha256":"$A64_SHA"},
  "arm32": {"build":"$A32_BUILD","identity":"$A32_IDENTITY","reproducibility":"$A32_REPRO","sha256":"$A32_SHA"},
...
if [ "$COMPLETE" -eq 1 ]; then
    if [ -s "$OUT/apkc-compile.txt" ]; then OLD_SHA="$(sha256sum "$OUT/apkc-compile.txt" | awk '{print $1}')"; cp "$OUT/apkc-compile.txt" "$ARCHIVE/apkc-compile.pre-${RUN_ID}.${OLD_SHA}.txt"; fi
    cp "$TRANSCRIPT" "$OUT/apkc-compile.txt"; cp "$A64_HDR" "$OUT/apkc-binary-arm64.txt"; cp "$A32_HDR" "$OUT/apkc-binary-arm32.txt"
...
if [ "$ALLOW_PARTIAL" -eq 1 ]; then exit 0; fi
exit 1
