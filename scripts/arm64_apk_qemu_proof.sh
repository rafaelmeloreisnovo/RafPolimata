    ...
    if [ $APKC_EXIT -eq 0 ]; then
        ...
        if unzip -p "$APK_OUT" classes.dex > /tmp/_apkc_proof_dex.bin 2>/dev/null \
           && [ -s /tmp/_apkc_proof_dex.bin ]; then
            {
              printf '# classes.dex SHA-1 — qemu proof commit %s %s\n' "$SHORT" "$DATE_UTC"
...
if command -v zipalign >/dev/null 2>&1 && command -v apksigner >/dev/null 2>&1; then
    ...
    if [ -f "$KEYSTORE" ]; then
        # Remove stale outputs so a failed signing step cannot be masked by a
        # previous run's file on a self-hosted or rerun CI environment.
...
# ── Update validation-summary.md with qemu proof overrides ────────────────
# apkc_validate.sh runs first and marks F2-F6 TOKEN_VAZIO on x86 hosts;
# override those rows with the evidence generated by this qemu proof.
VSUM="$OUT/validation-summary.md"
if [ -f "$VSUM" ] && [ "$PASS" -ge 2 ]; then
    APK_SZ="$(wc -c < "$APK_OUT" 2>/dev/null | tr -d ' ' || echo '?')"
    {
      printf '\n## ARM64 qemu proof overrides — commit %s %s\n\n' "$SHORT" "$DATE_UTC"
      echo "Gates promoted from TOKEN_VAZIO by arm64_apk_qemu_proof.sh:"
      echo ""
      ...
      if [ "$DEX_PROVEN" -eq 1 ]; then
          ...
          echo "| F5 | TOKEN_VAZIO | classes.dex ausente ou python3 ausente |"
      fi
