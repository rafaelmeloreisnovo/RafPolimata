...
case "$MODE" in extract-only|probe-only|adb-shell-pm|pm-local) ;; *) echo "provider_physical_chain_status=FAIL reason=UNKNOWN_MODE claim_allowed=false" >&2; exit 232;; esac
...
bash "$SEAL" "$OUT_DIR" provider-physical-tree.sha256 || exit 237
(cd "$OUT_DIR" && sha256sum -c provider-physical-tree.sha256 >/dev/null) || exit 238
