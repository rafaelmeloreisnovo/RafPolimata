...
TMP="$FINAL.tmp.$$"
emit(){
  rc=$1; overall=$2; reason=$3; snapshot=${4:-TOKEN_VAZIO}; installer=${5:-TOKEN_VAZIO}; expected=${6:-TOKEN_VAZIO}; observed=${7:-TOKEN_VAZIO}; seals=${8:-TOKEN_VAZIO}; bytes=${9:-TOKEN_VAZIO}
...
single_value(){ file=$1; key=$2; count=$(grep -c "^${key}=" "$file" 2>/dev/null || true); [ "$count" -eq 1 ] || return 1; value=$(grep "^${key}=" "$file" | cut -d= -f2-); [ -n "$value" ] || return 1; printf '%s' "$value"; }
is_sha256(){ case ${1:-} in (*[!0-9a-f]*|'') return 1;; (*) [ "${#1}" -eq 64 ];; esac; }
is_hex(){ case ${1:-} in (0x[0-9a-fA-F]*) return 0;; (*) return 1;; esac; }
is_uint(){ case ${1:-} in (*[!0-9]*|'') return 1;; (*) return 0;; esac; }
is_pos_uint(){ case ${1:-} in ([1-9]*) case $1 in (*[!0-9]*) return 1;; esac; return 0;; (*) return 1;; esac; }
...
is_hex "$seals" || emit 187 FAIL SEALS_MALFORMED "$snapshot" "$installer" "$expected" "$observed" "$seals" "$bytes"
seal_num=$((seals)); [ $((seal_num & 15)) -eq 15 ] || emit 187 FAIL REQUIRED_SEALS_INCOMPLETE "$snapshot" "$installer" "$expected" "$observed" "$seals" "$bytes"
is_pos_uint "$bytes" || emit 187 FAIL APK_BYTES_INVALID "$snapshot" "$installer" "$expected" "$observed" "$seals" "$bytes"
is_uint "$installer" || emit 185 FAIL INSTALLER_EXIT_NOT_INTEGER "$snapshot" "$installer" "$expected" "$observed" "$seals" "$bytes"
