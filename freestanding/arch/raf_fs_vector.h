...
/*
 * Branchless vector select. `mask` is one full vector containing 0xFFFFFFFF
 * for selected u32 lanes and 0 for preserved lanes. This is also the residual
 * primitive for fixed-vector ISAs without predicated memory: caller provides a
 * full accessible block and an explicit mask; no cleanup loop is synthesized.
 */
