...
# Required signing + keystore: signed and verified target is selected.
...
# Auto without keystore accepts only a raw APK that verifies.
...
# Explicit bypass preserves compatibility but cannot claim signing.
...
printf '%s\n' 'PASS: sign/install gate positive + required-sign + sign-fail + verify-fail + auto + explicit-bypass cases'
